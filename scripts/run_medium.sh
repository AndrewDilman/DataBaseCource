...
echo "=== Запуск среднего варианта (medium) ==="
...
docker cp "$SCRIPT_DIR/../generate_medium_data.sql" marketplace-db:/generate_medium.sql
docker exec marketplace-db psql -U admin -d marketplace -f /generate_medium.sql
docker exec marketplace-db rm -f /generate_medium.sql

echo "3. Миграция данных в Neo4j..."
bash "$PROJECT_DIR/neo4j/migrate_medium.sh"
