...
echo "Экспорт покупок (агрегированные orders)..."
...
docker exec marketplace-db psql -U admin -d marketplace -c "\copy (SELECT user_id, good_id, rating, comment FROM reviews) TO '/tmp/neo4j_export/reviews.csv' WITH CSV HEADER;"

# --- Копирование всех CSV в Neo4j ---
...
# --- Создание ограничений (уникальность узлов) ---
...
echo "Загрузка связей BOUGHT (покупатель → товар) ..."
docker exec -i marketplace-neo4j cypher-shell -u neo4j -p 12345678 <<'EOF'
...
echo "Загрузка связей REVIEWED (покупатель → товар с отзывом) ..."
docker exec -i marketplace-neo4j cypher-shell -u neo4j -p 12345678 <<'EOF'
